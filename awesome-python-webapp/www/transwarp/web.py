...
def _to_unicode(s, encoding='utf-8'):
	...
	True
	'''
	return s.decode('utf-8')# is this return s.decode(encoding) ?
...
def _build_regex(path):
	...
	re_list = ['^']
	var_list = []
	is_var = False#	print 'split:', _re_route.split(path)
...
class Request(object):
	...
	def headers(self):
		'''
		Get all HTTP headers with key as str and value as unicode. The headers names are 'XXX-XXX' uppercase.
...
class Response(object):
	...
	def status(self, value):
		...
		Traceback (most recent call last):
			...
		TypeError: Bad response code: ok
		>>> r.status = [1, 2, 3]
		Traceback (most recent call last):
			...
		...
		if isinstance(value, (int, long)):
			...
			if _RE_RESPONSE_STATUS.match(value):
				...
				raise TypeError('Bad response code: %s' % value)
		else:
